...
- **Membership:** `main_feed` and **public** named `text` channels implicitly include all users; DM channels have exactly two user members. Bots are members of `main_feed` by default; **bots are members of a DM or `text` channel only if explicitly added** (which publishes `member_add`/`member_remove` like any other membership change). Any user may create a `text` channel (`POST /channels {name, visibility?}`) and, on a public one, manage its bots (flat access — every user is a member). `notify_all_main` covers `main_feed` only; `text` channels are mention-notify only.
- **Private channels:** a `text` channel created with `visibility: "private"` has **no implicit members** — `ChannelMember` is the enforced wall, for humans and bots alike, checked on every read path (history, read state, member list, search, WS fan-out, push). Non-members get 403; `GET /channels` **omits** a private channel from an ordinary non-member's sidebar (RULED by plink, 2026-08-17, superseding the spec's original "existence is not hidden"), while an **admin** still gets the row — existence only, `member: false`, no unread, no snippet, and still no read access. The sidebar filter is derived from membership (`user_channel_ids`), not a second rule. Only its owner (`created_by`) may invite, kick, or add a bot; anyone may leave. There is **no admin god-view**: the app ships no in-product override. Channel-level ACL and message-level `privacy_flags` are independent walls that compose.
- **Deleting a channel:** `DELETE /channels/{id}` on a `text` channel (main_feed and DMs are refused) by its **owner or any admin** (RULED by plink, 2026-08-17 — deletion destroys content instead of handing out read access, so it is no god-view and invite/kick stay owner-only) **hard-deletes** the row, cascading to its members, messages, attachment rows and FTS index; attachment files on disk are left as orphans. Publishes `channel_delete` to everyone who could see the channel a moment earlier (everyone if public; if private, its members plus every admin, whose sidebar carried its bare row) — an audience computed **before** the delete, since afterwards nobody is a member.
...
## 13. v2 Backlog
- ~~**Dead-mount investigation**~~ **RESOLVED 2026-07-22.** The seq-138 lead was right: `disjorn-broker.service` had `RuntimeDirectory=` without `RuntimeDirectoryPreserve=`, so systemd deleted `/run/disjorn-broker` on every service stop — out from under the bind every running resident container holds to reach the socket. The container kept a bind to an unlinked directory and silently lost its hands. `RuntimeDirectoryPreserve=yes` was already live on the host but was **missing from the repo template**, so a reinstall via `harness/keyboard/04-broker.sh` would have regressed it; now landed in the template with the mechanism written down, plus a regression test (`test_unit_template_keeps_the_dead_mount_fix`). Related and still open: with the default `KillMode=control-group` a broker restart also kills any in-flight detached build and loses its narration — tracked with the `start-build` work, not here.
- **Bot unprompted participation** (per plink 2026-07-18): policy layer for bots joining conversation un-summoned — rate limits, spend budget, personality tuning. Design alongside AGENTHOOD.md budgets.
