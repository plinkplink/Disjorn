...
## 13. v2 Backlog
- **Dead-mount investigation** (TOP per plink 2026-07-21, #custodian seq 133): why resident containers come up on a stale /run/disjorn-broker directory bind after a broker bounce — second recurrence, different mechanism each time. Warm lead (seq 138): disjorn-broker.service has `RuntimeDirectory=` without `RuntimeDirectoryPreserve=`, so every stop deletes the dir under running containers' binds; candidate fix `RuntimeDirectoryPreserve=yes` + restart ordering. Summoned-Gable drives.
