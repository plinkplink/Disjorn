...
`seat surface: res-claudette deployed ref matches live verbs.toml (16 tools,
claudette.git disjorn-port:broker_tools.py)`, or `deployed ref DRIFT` naming
the verbs `not deployed`, `deployed, not listed` and `schema differs`. A kill-switch flip is not drift
...
`NOT CONFIGURED` with the reason. None of them prints as a match. Every
configured seat prints a line even when verbs.toml cannot be read.
...
`refresh-mirror` runs, so it can lag. The line says "deployed ref" because
it reads that ref, not the module her running process loaded. The two differ
when the script fails after its push, or runs with `--no-restart` and she has
not been restarted since. The script reports both cases when they happen. Gable's line says `not compared — shell
...
seat is accounted for. His line is the same when verbs.toml cannot be read,
because nothing about him is compared either way. The table is `metrics.SEAT_SURFACES`, overridden whole
